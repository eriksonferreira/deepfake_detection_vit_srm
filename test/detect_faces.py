...
def process_videos(videos, detector_cls: Type[VideoFaceDetector], selected_dataset, opt):
    detector = face_detector.__dict__[detector_cls](device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
    print(torch.device('cuda' if torch.cuda.is_available() else 'cpu'))

    dataset = VideoDataset(videos)
    
    loader = DataLoader(dataset, shuffle=False, num_workers=2, batch_size=1, collate_fn=new_fn)
    missed_videos = []
...
def main():
    ...
    parser.add_argument("--processes", help="Number of processes", default=6)
