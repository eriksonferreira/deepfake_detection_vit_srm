...
import numpy as np
from models.srm_conv import SRMConv2d_simple, SRMConv2dTransform
from models.sobel_conv import SobelConv2d
...
class MultiScaleBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("outro token", proj_cls_token)
        # cross attention
        outs = []
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, multi_conv=False, add_srm=False, add_sobel=False,**kwargs):
        super().__init__()

        print(f"ViT with {num_classes} classes.")
        self.num_classes = num_classes
        self.add_srm = add_srm
        ...
        # Instanciar SRMConv2dTransform apenas para o último branch (maior)
        self.srm = SRMConv2dTransform(inc=in_chans) if self.add_srm else None
        self.sobel = SobelConv2d() if self.add_sobel else None
    ...
    def forward_features(self, x):
        ...
        mean = torch.tensor([0.485, 0.456, 0.406], dtype=torch.float32, device=x.device)
        std = torch.tensor([0.229, 0.224, 0.225], dtype=torch.float32, device=x.device)
        # threshold=0.2
        for i in range(self.num_branches):
            # Redimensionar a imagem para o tamanho do branch atual
            x_ = torch.nn.functional.interpolate(x, size=(self.img_size[i], self.img_size[i]), mode='bicubic') if H != self.img_size[i] else x
            
            # Aplicar SRM e salvar em escala de cinza para o branch maior (posição [0])
            ###### IMPORNTANTE ######
            ##### AO TESTAR MODELOS COM FILTROS L-BRANCH OU S-BRANCH, DEVE-SE
            ##### MODIFICAR AQUI 0 OU 1 PARA QUE A MUDANÇA OCORRA NO BRANCH CORRETO
            #### 0 para L e 1 para S. Por enquanto não há implementação parametrizável.
            if self.add_srm and i == 0:
                x_ = self.srm(x_)  # Aplicar filtro SRM
                # print(f"Forma após SRM no branch maior (posição {i}): {x_.shape}")

                # Agrupar os canais em uma imagem em escala de cinza
                # combined_channels = x_[0].cpu().mean(dim=0)  # Calcula a média dos canais para combinar em escala de cinza
                # combined_channels = (combined_channels - combined_channels.min()) / (combined_channels.max() - combined_channels.min())  # Normalizar para [0, 1]
                
                # # Converte para imagem PIL em escala de cinza e salva
                # gray_img = to_pil(combined_channels.unsqueeze(0))  
                # gray_img.save(f"patches/branch_{i}_{self.init}_combined_gray.png")  
                x_ = self.sobel(x_)  # Aplicar filtro Sobel
                # print(f"Forma após SRM no branch maior (posição {i}): {x_.shape}")
            ...
            # Salvar a imagem original em cor para o branch menor (posição [1])
            # elif i == 1:
            #     # Inversão da normalização para o branch menor
            #     img_tensor = x_[0].to(x.device)  # Extrai o primeiro exemplo do batch
            #     img_tensor = img_tensor * std.to(img_tensor.device).unsqueeze(1).unsqueeze(2) + mean.to(img_tensor.device).unsqueeze(1).unsqueeze(2)
            #     img_tensor = torch.clamp(img_tensor, 0, 1)  # Garante que os valores estejam no intervalo [0, 1]
                
            #     # Converte para imagem PIL e salva em cores
            #     original_img = to_pil(img_tensor.cpu())  # Move para CPU para salvar como PIL
            #     original_img.save(f"patches/branch_{i}_{self.init}_original_color.png")  # Salva a imagem colorida original
...
def crossvit_15_dagger_224(pretrained=False, **kwargs):
    ...
    model.default_cfg = _cfg()
     ...
     # Modificar a última camada para classificação binária
    num_classes = 1
    
    # Obter o número de características de entrada da camada head original
    if hasattr(model, 'head'):
        in_features_0 = model.head[0].in_features if isinstance(model.head, nn.ModuleList) else model.head.in_features
        in_features_1 = model.head[1].in_features if isinstance(model.head, nn.ModuleList) else model.head.in_features
    else:
        raise AttributeError("O modelo não possui uma camada 'head'. Verifique a arquitetura do modelo.")

    # Modificar a última camada para classificação binária
    model.head = nn.ModuleList([nn.Linear(in_features_0, num_classes), nn.Linear(in_features_1, num_classes)])


    if pretrained:
        # Carregar o estado pré-treinado
        state_dict = torch.hub.load_state_dict_from_url(_model_urls['crossvit_15_dagger_224'], map_location='cpu')

        # Remover pesos da camada de classificação antiga
        state_dict = {k: v for k, v in state_dict.items() if not k.startswith('head')}

        # Carregar os pesos restantes
...
@register_model
def crossvit_18_dagger_224_srm(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=[240, 224],
                              patch_size=[12, 16], embed_dim=[224, 448], depth=[[1, 6, 0], [1, 6, 0], [1, 6, 0]],
                              num_heads=[7, 7], mlp_ratio=[3, 3, 1], qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), multi_conv=True, add_srm=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(_model_urls['crossvit_18_dagger_224'], map_location='cpu')
        model.load_state_dict(state_dict)
...
@register_model
def crossvit_18_dagger_224_sobel(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=[240, 224],
                              patch_size=[12, 16], embed_dim=[224, 448], depth=[[1, 6, 0], [1, 6, 0], [1, 6, 0]],
                              num_heads=[7, 7], mlp_ratio=[3, 3, 1], qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), multi_conv=True, add_sobel=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(_model_urls['crossvit_18_dagger_224'], map_location='cpu')
        model.load_state_dict(state_dict)
...
@register_model
def crossvit_18_dagger_224_srm_L_sobel_S(pretrained=False, **kwargs):
    model = VisionTransformer(img_size=[240, 224],
                              patch_size=[12, 16], embed_dim=[224, 448], depth=[[1, 6, 0], [1, 6, 0], [1, 6, 0]],
                              num_heads=[7, 7], mlp_ratio=[3, 3, 1], qkv_bias=True,
                              norm_layer=partial(nn.LayerNorm, eps=1e-6), multi_conv=True, add_srm=True, add_sobel=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url(_model_urls['crossvit_18_dagger_224'], map_location='cpu')
        model.load_state_dict(state_dict)
