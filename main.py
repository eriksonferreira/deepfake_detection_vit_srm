...
# from datasetsV2NEW import build_dataset
...
def get_args_parser():
    ...
    parser.add_argument('--nb_classes', type=int, default=1000, metavar='PCT',
    ...
    parser.add_argument('--data-path', default=os.path.join(os.path.expanduser("~"), 'datasets/image_cls/imagenet1k/'), type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='IMNET', choices=['CIFAR10', 'CIFAR100', 'IMNET', 'INAT', 'INAT19','DFDC'],
    ...
    parser.add_argument('--is_experiment', action='store_true', help='just a fast test')
...
def main(args):
    ...
    save_args_to_file(args, args_file)
    ...
    if args.eval:
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['new_acc']:.2f}%")
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_stats = evaluate(data_loader_val, model, device, num_tasks, distributed=True, amp=args.amp)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['new_acc']:.2f}%")
        max_accuracy = max(max_accuracy, test_stats["new_acc"])
        print(f'Max accuracy: {max_accuracy:.2f}%')

        if args.output_dir:
            checkpoint_paths = [output_dir / 'checkpoint.pth']
            if test_stats["new_acc"] == max_accuracy:
